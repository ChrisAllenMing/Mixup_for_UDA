...
def train(epoch, model):
    ...
    while i <= len_source_loader:
        ...
        # feature-level mixup
        mix_ratio = np.random.beta(opt.alpha, opt.alpha)
        mix_ratio = round(mix_ratio, 2)
        # clip the mixup ratio
        if (mix_ratio >= 0.5 and mix_ratio < (0.5 + opt.clip_thr)):
            mix_ratio = 0.5 + opt.clip_thr
        if (mix_ratio > (0.5 - opt.clip_thr) and mix_ratio < 0.5):
            mix_ratio = 0.5 - opt.clip_thr

        dlabel_mix = Variable((torch.ones(opt.batch_size) * mix_ratio).long().cuda())
        ...
        domain_loss_total = domain_loss_src + domain_loss_tgt + domain_loss_mix * opt.mix_weight
