    ...
    def train(self):
        ...
        for epoch in range(self.opt.nepochs):
            ...
            for i, (datas, datat) in enumerate(itertools.izip(self.source_trainloader, self.target_trainloader)):
                ...
                # Visualization
                if i == 1:
                    vutils.save_image((src_gen.data/2)+0.5, '%s/source_generation/source_gen_%d.png' %(self.opt.outf, epoch))
                    vutils.save_image((tgt_gen.data/2)+0.5, '%s/target_generation/target_gen_%d.png' %(self.opt.outf, epoch))
                    vutils.save_image((mix_gen.data/2)+0.5, '%s/mix_generation/mix_gen_%d.png' %(self.opt.outf, epoch))
                    vutils.save_image((mix_samples.data/2)+0.5, '%s/mix_images/mix_samples_%d.png' %(self.opt.outf, epoch))
